...
<div class="border-chat-border flex items-center justify-between border-b border-solid px-4 py-3 md:px-5 md:py-4">
    <div class="flex items-center gap-2.5">
        ...
        <span class="text-chat-text-muted text-micro tracking-[0.22em] whitespace-nowrap uppercase">
            {statusConfig.label}
        </span>
    </div>
    <div class="flex items-center gap-1">
        ...
        >
            ...
            {#if historyCount > 0}
                <span
                    class="bg-chat-accent text-chat-bg text-micro absolute -top-0.5 -right-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full px-1 font-semibold tabular-nums"
                >
                    {historyCount}
                </span>
            {/if}
