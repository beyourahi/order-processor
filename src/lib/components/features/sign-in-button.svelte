<!--
    Logged-out counterpart to <User>, rendered inline inside the invisible <Navbar>: a pill inviting
    (not forcing) sign-in. Auth is optional — guests use the full app with localStorage-backed
    settings; signing in adds cross-device sync + the AI Copilot. Mirrors the avatar's
    `sleek border-hair bg-card` treatment so the corner reads consistently whether signed in or out.
-->
<script lang="ts">
