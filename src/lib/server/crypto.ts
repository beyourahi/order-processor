/**
 * AES-GCM helpers for at-rest encryption of the user's bring-your-own
 * Cloudflare API token.
 *
 * BLOB layout in `user_settings.cloudflare_token_encrypted`:
 *   [12-byte IV] || [ciphertext + 16-byte auth tag]
 *
 * Uses WebCrypto only (no Node `crypto`); safe in any Workers context.
 * Key is derived once from env.TOKEN_ENCRYPTION_KEY (base64, 32 bytes).
 */
...
export async function encryptToken(plaintext: string, key: CryptoKey): Promise<Uint8Array> {
...
export async function decryptToken(blob: Uint8Array, key: CryptoKey): Promise<string> {
    if (blob.length <= IV_LENGTH_BYTES) {
        throw new Error(`decryptToken: blob too short (${blob.length} bytes); expected > ${IV_LENGTH_BYTES}`);
    }
...
/** Display mask: `<first 10 chars>...`. Never returns the full secret. */
