...
| ID | File | Finding | Resolution |
|----|------|---------|------------|
| DS-001 | `src/app.css` | Emerald green used but not tokenized | Added `--color-courier-accent: oklch(0.69 0.17 162)` to `@theme` block |
| DS-002 | Throughout | `zinc-{700,800,900}` used directly | **DEFERRED** — touches >15 files, risk outweighs benefit |
| DS-003 | `login/+page.svelte`, `order-processor.svelte` | `red-400`/`red-500/10` instead of destructive token | Replaced with `text-destructive` / `bg-destructive/10` |
...
| COMP-003 | `+error.svelte:17` | Status code in `<h1>` (semantically wrong) | Status code moved to `<p>`; message promoted to `<h1>` |
...
| API-005 | `logout/+server.ts` | No error handling on cookie operations | Wrapped in try/catch; returns `json({ success: false })` on failure |
